...
def annealing(initial, initialTemp = 1, Nswap = 10):
    """
    ...
    for _ in range(Nswap): #Chooses a random neighbour
        ...
        distanceDiff = - Fitness(neighbour) + Fitness(solution[:]) #Compares the total distances of the two solutions
    ...
    return solution, Fitness(solution), T
...
def PT(routes, temps):
    ...
    for i in range(len(temps)-1):
        ...
        DeltaE =  Fitness(routes[i+1]) - Fitness(routes[i])
        if uniform(0,1) < np.exp(DeltaT*DeltaE):
            temps[i], temps[i+1] = temps[i+1], temps[i]
    return routes, temps


def simulationFinal(Ntot, Nswap, Temps, paths):
    ...
    for _ in range(int(Ntot / Nswap)):  # This will be the number of swap trials with PT
        ...
        for i in range(len(Temps)):
            bestRoute, bestRouteDistance, T = annealing(paths[i], initialTemp=Temps[i], Nswap=Nswap)
            routes.append(bestRoute[:])
            Temps[i] = T

        # PT will propose the temperature swaps
        paths, Temps = PT(routes, Temps)
...
paths, Temps, tempForPlot = simulationFinal(Ntot, Nswap, Temps, paths)
